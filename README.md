...
- **Gemini API (Google AI Studio)** — Google's multimodal foundation model. We send it a dynamically-built list of whatever a citizen actually submitted (text/photo/audio/video, any combination) in a single call, and it returns structured JSON: transcription + translation of native-language audio/text (13 Indian languages), visual hazard analysis on photos/video, pollutant type, severity score (1–5), plain-English summary, and recommended action. Model fallback chain (`gemini-2.5-flash` → `gemini-2.5-flash-lite` → `gemini-2.0-flash`) for reliability under quota limits.
